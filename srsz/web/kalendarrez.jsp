...
<script src="/srsz/js/gcal.js"></script>
...
$("#calendar").fullCalendar("refetchEvents");
                    ...
                    var calendar = $('#calendar').fullCalendar({
                        ...
                        height: 'auto',
                        googleCalendarApiKey: 'AIzaSyAnn7c6UqIsbA_X-YXonwvG3wk7OTyhUXg',

                        eventSources:  [
                            {url:'<portlet:resourceURL><portlet:param name="jspPage" value="/pliki/wlasc1.jsp" /></portlet:resourceURL>&wlasc=' + <%=usr%>
                                },
                                        {url:'<portlet:resourceURL><portlet:param name="jspPage" value="/pliki/prosbyy.jsp" /></portlet:resourceURL>&wlasc=' + <%=usr%>,color:'green',selectable:false}
                     ],
                            ...
                            if (eventStart >= ntoday && eventEnd >= ntoday) {
                                ...
                                //  opisup = event.title.toString();
                                 ...
                                 if(prosba ==1){
                                    ...
                                    element.find("div.fc-title").after($("<span class=\"fc-icons\"></span>").html("<img src=\"/srsz/ikona.png\" onclick=\"usunpro(" + idd + ");\" />"));
                                }
                                ...
                                if (event.prosba==1){
                                    element.css('background-color','#006633');
                                    element.css('border-color','#003300');
                                }
        ...
        function edycja(typupp, iddd, startupp, endupp, idzz) {
         ...
         function usunpro(idd) {
            ...
            $("#button4").on('click', function (e) {
                e.preventDefault();
                $.ajax({
                    url: '<portlet:resourceURL><portlet:param name="jspPage" value="/usunProsbaAkcja.jsp" /></portlet:resourceURL>',
                    type: "GET",
                    data: "&prosba=" + idd,
                    success: function (json) {
                        $("#usunrez").dialog("close");
